...
public class BuyHigh implements Tip {
    ...
    public JFreeChart buildChart(TimeSeries series) {
        ...
        tradingRecord = seriesManager.run(strategy, OrderType.BUY, PrecisionNum.valueOf(1.0));
    ...
    public String getStrategyAnalysis() {
        ...
        sb.add("Total profit: " + totalProfit.calculate(series, tradingRecord));
        // Number of bars
        sb.add("Number of bars: " + new NumberOfBarsCriterion().calculate(series, tradingRecord));
        // Average profit (per bar)
        sb.add("Average profit (per bar): " + new AverageProfitCriterion().calculate(series, tradingRecord));
        // Number of trades
        ...
        // Maximum drawdown
        sb.add("Maximum drawdown: " + new MaximumDrawdownCriterion().calculate(series, tradingRecord));
        // Reward-risk ratio
        sb.add("Reward-risk ratio: " + new RewardRiskRatioCriterion().calculate(series, tradingRecord));
        // Total transaction cost
        sb.add("Total transaction cost (from $1000): "
                + new LinearTransactionCostCriterion(5, 0.005).calculate(series, tradingRecord));
        // Buy-and-hold
        sb.add("Buy-and-hold: " + new BuyAndHoldCriterion().calculate(series, tradingRecord));
        ...
        // End cash
        sb.add("END CASH: " + cashFlow.getValue(series.getEndIndex()));
        // ATR
        // Want to put this on the legend
        ATRIndicator atr = new ATRIndicator(series, 14);
        sb.add("ATR: " + atr.getValue(series.getBarCount()-1));
        return sb.toString();
    }
